...
from utils.func import load_config, get_pictures, apply_hough_transformation, group_lines, plot_results, compose_court_data
...
def run():
    ...
    path = Path(f'prod_results/{now_str}')
    path.mkdir(exist_ok=True, parents=True)

    params = load_config('prod_params.json', ImageParams)
    pics = get_pictures('prod_pictures')['rgb']


    for pic, param in zip(pics, params):
        ...
        further_outer_baseline_intersection, last_local_line = candidates.find_further_outer_baseline_intersection(closer_outer_baseline_intersecetion, used_line, param.canny_thresh.lower , param.canny_thresh.upper, offset=param.offset)
        ...
        further_outer_sideline = candidates.find_further_doubles_sideline(further_outer_baseline_point, last_local_line, param.offset, param.extra_offset, param.bin_thresh, param.surface_type)
        further_outer_netline_point = further_outer_sideline.intersection(netline, pic).point


        closer_inner_baseline_point, further_inner_baseline_point = candidates.scan_endline(baseline, netline, closer_outer_baseline_point, further_outer_netline_point, closer_outer_netline_point, further_outer_baseline_point, param.bin_thresh_endline_scan.baseline, param.canny_thresh.lower, param.canny_thresh.upper, param.max_line_gap, searching_line = 'base')


        closer_inner_netline_point, further_inner_netline_point = candidates.scan_endline(baseline, netline, closer_outer_baseline_point, further_outer_netline_point, closer_outer_netline_point, further_outer_baseline_point, param.bin_thresh_endline_scan.netline, param.canny_thresh.lower, param.canny_thresh.upper, param.max_line_gap, searching_line = 'net')
